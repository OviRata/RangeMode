...
int main(){
  ...
  cout<<"Building time: "<<time<<" microseconds "<<" = "<< ( time/(float)1000000*60 )<<" minutes \n";
  ...
  for(int k=0; k<=6; k++){
    ...
    for(int q=1; q<=cnt; q++){
      ...
      long long currentDuration = std::chrono::duration_cast<std::chrono::microseconds>( end - start ).count();
  ...
  for(int logL=0, l=1; l<8000000; logL++, l*=2 ){
    long long totalTime = 0;
    int cnt = 10000000;
    for(int q=1; q<=cnt; q++){
      auto pr = generateQueryForLength(n, l);
